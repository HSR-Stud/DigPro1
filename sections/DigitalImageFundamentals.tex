\section{Digital image fundamentals \buch{p.47}}
  \begin{center}
    intensity values $L = 2^k$  with k-bit\\
    bits to save $b = M \cdot N \cdot k$
  \end{center}
\subsection{Image Interpolation \buch{p.77}}
\begin{itemize}
...
\subsection{Some basic relationships between pixels \buch{p.79}}
\subsubsection{Neighbors of a pixel \buch{p.79}}
...
\subsubsection{Adjacency, connectivity, regions and boundaries \buch{p.79}}
\begin{description}
...
\subsubsection{Distance measures / Neighbors \buch{p.82}}
...
\subsection{Math tools \buch{p.83}}
\subsubsection{Noise reduction \buch{p.86}}
Image with noise
\begin{equation}
g(x,y) = f(x,y) + \eta(x,y)
\end{equation}
...
\subsubsection{Set and Logical Operations \buch{p.91}}
\begin{tabular}{|l|l|l|}
  \hline
...
\subsubsection{Neighborhood operations \buch{p.99}}
Averaging of neighborhood $S_{xy}$
\begin{equation}
g(x,y) = \frac{1}{mn} \sum_{(r,c)\in S_{xy}}f(r,c)
\end{equation}

\subsubsection{Geometric spatial Transformations \buch{p.100}}
...
\subsubsection{Image registration \buch{p.103}}
\begin{itemize}
